...
<offers>
    ...
    <offer>
        ...
        <sku>{{var sku}}</sku>
        <title>{{var offer_name}}</title>
        <url>{{var url}}</url>
        {{foreach delivery_times as row}}<delivery{{if row.context}} context="{{var row.context}}"{{/if}}>{{var row.delivery_time}}</delivery>
        ...
        {{/foreach}}{{foreach shipping_comments as row}}<shippingComment{{if row.context}} context="{{var row.context}}"{{/if}}>{{var row.shipping_comment}}</shippingComment>
        {{/foreach}}<category>{{var category_path}}</category>
        {{if manufacturer}}<brand>{{var manufacturer}}</brand>
        {{/if}}{{if oen}}<oen>{{var oen}}</oen>
        {{/if}}{{if kba}}<kba>{{var kba}}</kba>
        {{/if}}{{if ean}}<ean>{{var ean}}</ean>
        {{/if}}{{if han}}<han>{{var han}}</han>
        {{/if}}{{if pzn}}<pzn>{{var pzn}}</pzn>
        {{/if}}<description>{{var description:970}}</description>
